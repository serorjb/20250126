...
fx['TICKER'] = fx['TICKER'].fillna('CAD')
...
univ_map = pd.read_csv('data/univ_map.csv', parse_dates=['EFFFROM', 'EFFTHRU'], date_format='%Y-%m-%d')
ven_map = pd.read_csv('data/ven_map.csv', parse_dates=['EFFFROM', 'EFFTHRU'], date_format='%Y-%m-%d')
...
univ_map['EFFTHRU'] = pd.to_datetime(univ_map['EFFTHRU'], errors='coerce', format='%Y-%m-%d').fillna(pd.Timestamp('2262-04-11'))
ven_map['EFFTHRU'] = pd.to_datetime(ven_map['EFFTHRU'], errors='coerce', format='%Y-%m-%d').fillna(pd.Timestamp('2262-04-11'))
...
def fit_hmm(volatility_data, n_states=2):
    # note make sure you are using the latest version of hmmlearn, or it won't converge
